...
if(isset($_REQUEST["mode"]))
     $mode=$_REQUEST["mode"]; else 
     $mode="";
if(isset($_REQUEST["search"]))   // search 쿼리스트링 값 할당 체크
$search=$_REQUEST["search"]; else 
    $search="";
if(isset($_REQUEST["find"]))
    $find=$_REQUEST["find"]; else
    $find="";
if($mode=="search") {
	if(!$search) {
		?>
	...
	$sql="select * from phptest.concert where $find like '%$search%' order by num desc";
} else {
	$sql="select * from phptest.concert order by num desc";
}
try {
	$stmh = $pdo->query($sql);
	$count=$stmh->rowCount();
